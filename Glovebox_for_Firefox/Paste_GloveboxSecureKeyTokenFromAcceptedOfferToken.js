...
function Paste_GloveboxSecureKeyTokenFromAcceptedOfferToken(request, sender, sendResponse) {
	

    var replacementStr = request.Paste_GloveboxSecureKeyTokenFromAcceptedOfferToken_text;
    console.log("Paste_GloveboxSecureKeyTokenFromAcceptedOfferToken.js JSON(request): " + JSON.stringify(request));
...
var startNode = sel.getRangeAt(i).startContainer;
    ...
    console.log("startNode.textContent: " + nodetext);
	var position = sel.getRangeAt(i).startOffset;
    console.log(" sel.getRangeAt(i).startOffset : " + position );
...
browser.runtime.onMessage.addListener(Paste_GloveboxSecureKeyTokenFromAcceptedOfferToken);
